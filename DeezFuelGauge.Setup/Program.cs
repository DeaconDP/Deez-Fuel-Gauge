...
{
    Console.Error.WriteLine("Deez Fuel Gauge setup is only supported on macOS.");
    return 1;
}

var logDir = Path.Combine(
    Environment.GetFolderPath(Environment.SpecialFolder.UserProfile),
    "Library", "Logs", "DeezFuelGauge");
...
static void ShowMessage(string message)
{
    try
    {
        Process.Start(new ProcessStartInfo("/usr/bin/osascript")
        {
            ...
            {
                "-e",
                $"display dialog {Quote(message)} buttons {{\"OK\"}} default button \"OK\" with title \"Deez Fuel Gauge\""
            },
